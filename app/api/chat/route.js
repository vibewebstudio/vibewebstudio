    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:24px;background:#0e0e13;color:#e8e6f0;border-radius:12px">
          <h2 style="color:#bb9eff;margin-top:0">New Contact Form Submission (via Chat)</h2>
          <table style="width:100%;border-collapse:collapse">
            <tr><td style="padding:8px 0;color:#9f9dac;width:140px">Name</td><td style="padding:8px 0">${safeName}</td></tr>
            <tr><td style="padding:8px 0;color:#9f9dac">Email</td><td style="padding:8px 0"><a href="mailto:${safeEmail}" style="color:#00cffc">${safeEmail}</a></td></tr>
            ${safeCompany     ? `<tr><td style="padding:8px 0;color:#9f9dac">Business</td><td style="padding:8px 0">${safeCompany}</td></tr>` : ""}
            ${safeProjectType ? `<tr><td style="padding:8px 0;color:#9f9dac">Project Type</td><td style="padding:8px 0">${safeProjectType}</td></tr>` : ""}
            ${safeBudget      ? `<tr><td style="padding:8px 0;color:#9f9dac">Budget</td><td style="padding:8px 0">${safeBudget}</td></tr>` : ""}
          </table>
