      ...
      <div className="max-w-5xl mx-auto" style={{ position:"relative", zIndex:1 }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-14">
          {MEMBERS.map((m, i) => (
            <div
              ...
              style={{ background:m.bg, border:`1px solid ${m.border}`, cursor:"default" }}
            >
              <div className="member-avatar inline-flex items-center justify-center mb-6 rounded-2xl"
                style={{ width:72, height:72, background:`linear-gradient(135deg, ${m.bg}, rgba(255,255,255,0.04))`, border:`1.5px solid ${m.border}`, fontSize:"2rem", boxShadow:`0 0 24px ${m.glow}` }}>
                {m.emoji}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {m.skills.map((skill) => (
                  <span key={skill} className="text-xs font-semibold px-3 py-1.5 rounded-full"
                    style={{ background:"rgba(255,255,255,0.05)", border:"1px solid rgba(72,71,77,0.25)", color:"var(--c-on-sv)" }}>
                    {skill}
                  </span>
                ))}
              </div>
        ...
        <div className="text-center reveal"
          ...
          <p className="font-headline font-bold text-lg mb-6" style={{ color:"var(--c-on-surface)" }}>
            &ldquo;We focus on simple, effective websites that actually help businesses grow.&rdquo;
          </p>
