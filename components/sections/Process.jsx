          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative z-10 stagger-children">
            {STEPS.map((s, i) => (
              <div
                key={s.title}
                {...tilt}
                className={`step-card premium-card rounded-2xl p-8 text-center reveal reveal-delay-${i + 1}`}
                style={{ background:"var(--c-surface)", border:"1px solid rgba(72,71,77,0.15)" }}
              >
                <div
                  className="w-14 h-14 rounded-full mx-auto mb-6 flex items-center justify-center font-headline font-black text-xl"
                  style={s.gradient
